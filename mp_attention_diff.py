...
def mp_attention_diff(cur_qtr,prev_qtr):
    ...
    for fuid_sublist in fuid_keys:
        p1 = multiprocessing.Process(target=diffuse_attn, args=(fuid_sublist,prev_qtr,cur_qtr,iter,))
        mps.append(p1)
    ...
    if len(diff_mats) > 0:
        ...
        for iter in range(1,len(diff_mats)):
            ...
            mat2 = np.array(hf1["diffusion_matrix"])

            rslt = rslt.__add__(mat2)
            os.remove(mat_loc+diff_mats[iter])

        hf = h5py.File(mat_loc + "diffusion_qtr_"+str(cur_qtr).replace("-","_")+".h5", 'w')
        hf.create_dataset('diffusion_matrix', data=rslt)
        hf.close()
